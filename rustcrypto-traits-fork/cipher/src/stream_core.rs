...
#[inline]
fn into_chunks<T, N: ArrayLength>(buf: &mut [T]) -> (&mut [GenericArray<T, N>], &mut [T]) {
    use core::slice;
    if N::USIZE <= 1 {
        return (&mut [], buf);
    }
...
struct WriteBlockCtx<'a, BS: ArrayLength> {
    block: &'a mut Block<Self>,
}
impl<'a, BS: ArrayLength> BlockSizeUser for WriteBlockCtx<'a, BS> {
    type BlockSize = BS;
}
impl<'a, BS: ArrayLength> StreamClosure for WriteBlockCtx<'a, BS> {
    #[inline(always)]
...
struct WriteBlocksCtx<'a, BS: ArrayLength> {
    blocks: &'a mut [Block<Self>],
}
impl<'a, BS: ArrayLength> BlockSizeUser for WriteBlocksCtx<'a, BS> {
    type BlockSize = BS;
}
impl<'a, BS: ArrayLength> StreamClosure for WriteBlocksCtx<'a, BS> {
    #[inline(always)]
    fn call<B: StreamBackend<BlockSize = BS>>(self, backend: &mut B) {
...
struct ApplyBlockCtx<'inp, 'out, BS: ArrayLength> {
    block: InOut<'inp, 'out, Block<Self>>,
}

impl<'inp, 'out, BS: ArrayLength> BlockSizeUser for ApplyBlockCtx<'inp, 'out, BS> {
    type BlockSize = BS;
}

impl<'inp, 'out, BS: ArrayLength> StreamClosure for ApplyBlockCtx<'inp, 'out, BS> {
    #[inline(always)]
...
struct ApplyBlocksCtx<'inp, 'out, BS: ArrayLength> {
    blocks: InOutBuf<'inp, 'out, Block<Self>>,
}

impl<'inp, 'out, BS: ArrayLength> BlockSizeUser for ApplyBlocksCtx<'inp, 'out, BS> {
    type BlockSize = BS;
}

impl<'inp, 'out, BS: ArrayLength> StreamClosure for ApplyBlocksCtx<'inp, 'out, BS> {
    #[inline(always)]
