...
pub trait UniversalHash: BlockSizeUser + Sized {
    ...
    fn update(&mut self, blocks: &[Block<Self>]) {
        struct Ctx<'a, BS: ArrayLength> {
            blocks: &'a [Block<Self>],
        }

        impl<'a, BS: ArrayLength> BlockSizeUser for Ctx<'a, BS> {
            type BlockSize = BS;
        }

        impl<'a, BS: ArrayLength> UhfClosure for Ctx<'a, BS> {
            #[inline(always)]
...
#[inline(always)]
fn to_blocks<T, N: ArrayLength>(data: &[T]) -> (&[GenericArray<T, N>], &[T]) {
