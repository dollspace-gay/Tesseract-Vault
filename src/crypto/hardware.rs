...
// `unused_unsafe` silences stable-rustc's complaint that __cpuid is no longer
// unsafe; nightly (used by the fuzzer for -Zsanitizer) still requires unsafe.
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[allow(unused_unsafe)]
...
pub fn benchmark_aes_gcm(data_size: usize, iterations: u32) -> BenchmarkResult {
    ...
    // codeql[rust/hard-coded-cryptographic-value] - Benchmark requires deterministic inputs
    let key = [0x42u8; 32];
    // codeql[rust/hard-coded-cryptographic-value] - Benchmark requires deterministic inputs
